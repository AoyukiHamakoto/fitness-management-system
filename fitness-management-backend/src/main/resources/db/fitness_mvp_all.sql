-- =============================================================================
-- fitness_mvp 一键初始化（合并：fitness_mvp_schema.sql + seed_demo_user.sql
--       + alter_punch_record_checkin_columns.sql 之「条件化」旧库补丁）
-- MySQL 8.0+ | utf8mb4 / utf8mb4_unicode_ci
-- 用法示例：mysql -uroot -p --default-character-set=utf8mb4 < fitness_mvp_all.sql
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- 演示账号（BCrypt 与后端 PasswordEncoder 一致；密码 demo123）
-- README 中的 root/123456 为 MySQL 连接账号，非本站登录账号。
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- 附录：旧版 punch_record 缺字段/缺索引时补齐（全新库上文已含，本节多为 no-op）
-- 源自 alter_punch_record_checkin_columns.sql，改为条件执行避免重复报错
-- =============================================================================
USE `fitness_mvp`;
SET @db := DATABASE();
...
SET @sql := (
  SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'punch_record' AND COLUMN_NAME = 'actual_sets') = 0,
    'ALTER TABLE `punch_record` ADD COLUMN `actual_sets` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT ''实际完成组数'' AFTER `punch_date`',
    'SELECT 1'
  )
);
...
SET @sql := (
  SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'punch_record' AND COLUMN_NAME = 'actual_weight') = 0,
    'ALTER TABLE `punch_record` ADD COLUMN `actual_weight` DECIMAL(6, 2) NOT NULL DEFAULT 0.00 COMMENT ''实际使用重量（kg）'' AFTER `actual_sets`',
    'SELECT 1'
  )
);
PREPARE stmt FROM @sql;
...
SET @sql := (
  SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
     WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'punch_record' AND INDEX_NAME = 'uk_punch_user_detail_date') = 0,
    'ALTER TABLE `punch_record` ADD UNIQUE KEY `uk_punch_user_detail_date` (`user_id`, `plan_detail_id`, `punch_date`)',
    'SELECT 1'
  )
);
PREPARE stmt FROM @sql;
